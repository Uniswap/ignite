...
export type WorkflowPromoteRequest =
  ...
  | { mode: 'apply'; previewId: string; target: { repoPathOrUrl: string; name: string }; plan?: DeploymentPlan; runId?: string; tagChoiceBySourceId?: Record<string, string>; bookChoices?: Record<string, WorkflowPromotionBookChoice>; overwrite?: boolean; hooks: string[]; adoptRunIds?: string[] };
export type WorkflowPromoteData =
  | { mode: 'preview'; previewId: string; sources: WorkflowPromotionSourcePreview[]; referencedEntries: WorkflowPromotionBookPreview[]; nameCollision: boolean }
...
const PromotionApplyRequestSchema = z.object({
  ...
  bookChoices: z.record(z.string().regex(/^[a-z0-9][a-z0-9-]{0,63}$/), z.discriminatedUnion('action', [z.object({ action: z.literal('keep-repo') }).strict(), z.object({ action: z.literal('copy-under-new-name'), name: z.string().regex(/^[a-z0-9][a-z0-9-]{0,63}$/) }).strict()])).optional(),
