...
import { BookPointerSchema, DeploymentPlanSchema, type DeploymentPlan, type Hex } from './deployments.js';
import { AddressBookEntrySchema, type AddressBookEntry } from './addressBook.js';
...
export type ContractTypeWorkflowSource = {
  id: string;
  origin: 'contract-type';
  contractName: string;
} & WorkflowContractTypeSourceRef;
...
export type WorkflowPluginSource =
  ...
  | {
      ...
      url: string;
      ref?: string;
      track?: { mode: 'release'; version: string } | { mode: 'branch'; branch: string } | { mode: 'commit' };
      commit?: string;
    };
...
export type WorkflowDeployStrategy =
  ...
  | {
      kind: 'create2';
      salt: string;
      saltPerChain?: Record<string, string>;
      acknowledgeDeployed?: Record<string, { predictedAddress: string; initcodeHash: string }>;
    }
  | {
      kind: 'plugin';
      pluginId: string;
      ...
      salt?: string;
      saltPerChain?: Record<string, string>;
      prepared?: Record<string, { initcodeHash: string; predictedAddress: string }>;
      acknowledgeDeployed?: Record<string, { predictedAddress: string; initcodeHash: string }>;
    };
export type WorkflowStep =
  | {
      ...
      argsPerChain?: Record<string, Record<string, unknown>>;
      value?: string;
      valuePerChain?: Record<string, string>;
      gasOverrides?: WorkflowGasOverrides;
      gasOverridesPerChain?: Record<string, Partial<WorkflowGasOverrides>>;
      strategy?: WorkflowDeployStrategy;
      libraries?: Record<string, WorkflowLibraryBinding>;
      librariesPerChain?: Record<string, Record<string, WorkflowLibraryBinding>>;
      wraps?: { stepId: string; contractTypePluginId: string };
  ...
  | {
      ...
      targetPerChain?: Record<string, WorkflowCallTarget>;
      signature?: string;
      payable?: boolean;
      args?: Record<string, unknown>;
      argsPerChain?: Record<string, Record<string, unknown>>;
...
export interface WorkflowGasOverrides {
  gasLimit?: string;
  maxFeePerGas?: string;
...
export type WorkflowSourceReadiness =
  | { id: string; status: 'ready' | 'cloning' | 'compiling' }
  | {
      id: string;
      status: 'failed';
      reason: string;
      code?: 'ARTIFACT_NOT_FOUND' | 'FRAMEWORK_MISSING' | 'LIFECYCLE_FAILED';
      artifactPath?: string;
    };
export type WorkflowPluginReadiness = { id: string; status: 'installed'; installedVersion: string } | { id: string; status: 'version-mismatch'; installedVersion: string } | { id: string; status: 'missing' } | { id: string; status: 'untrusted'; installedVersion: string } | { id: string; status: 'wrong-type'; installedVersion: string };
export interface WorkflowInstallResult {
  sources: WorkflowSourceReadiness[];
...
export interface InstalledWorkflowsFile {
  schemaVersion: 1;
  records: InstalledWorkflowRecord[];
  quarantinedAt?: string;
}

export interface WorkflowPromotionSourcePreview {
  sourceId: string;
  origin: string;
  commit: string;
  tagChoices: string[];
...
export interface WorkflowPromotionBookPreview {
  name: string;
  entry: AddressBookEntry;
  resolutions: Record<string, Hex>;
  source: 'local' | 'repo';
  bookHash: string;
  targetEntry?: AddressBookEntry;
  conflict: boolean;
}
...
export type WorkflowPromoteRequest =
  | {
      mode: 'preview';
      target: { repoPathOrUrl: string; name: string };
      plan?: DeploymentPlan;
      runId?: string;
    }
  | {
...
export type WorkflowPromoteData =
  | {
      mode: 'preview';
      previewId: string;
      sources: WorkflowPromotionSourcePreview[];
      referencedEntries: WorkflowPromotionBookPreview[];
...
export interface WorkflowCheckUpdatesRequest {
  repoPathOrUrl: string;
  name: string;
}
...
export interface WorkflowCheckUpdatesData {
  docHash: string;
  sources: WorkflowSourceUpdate[];
  plugins: WorkflowPluginUpdate[];
}
...
export type WorkflowAttemptStatus =
  ...
  | {
      status: 'failed' | 'interrupted';
      error: string;
      failedSources?: NonNullable<InstalledWorkflowRecord['lastAttempt']>['failedSources'];
      atDocHash: string;
    };
...
export interface WorkflowInstallDiff {
  ...
  sourcesRenamed: Array<{
    from: string;
    to: string;
    detail: WorkflowSourceDetail;
  }>;
  versionsChanged: Array<{
    detail: WorkflowSourceDetail;
...
function payloadIssue(value: unknown, ctx: z.RefinementCtx, path: (string | number)[]): void {
  ...
  if (bytes > MAX_STEP_PAYLOAD_BYTES)
    ctx.addIssue({
      code: 'custom',
      message: `payload exceeds ${MAX_STEP_PAYLOAD_BYTES} bytes`,
      path,
    });
  if (jsonDepth(value) > MAX_JSON_DEPTH)
    ctx.addIssue({
      code: 'custom',
      message: `payload exceeds JSON depth ${MAX_JSON_DEPTH}`,
      path,
    });
}
...
const AddressSchema = z.string().regex(ADDRESS) as z.ZodType<Hex>;
const Hex32Schema = z.string().regex(HEX32);
const ValueRefSchema = z
  .object({
    $ref: z.object({ kind: z.literal('step'), stepId: z.string().min(1) }).strict(),
  })
  .strict() satisfies z.ZodType<WorkflowValueRef>;
const GasOverridesSchema = z
  .object({
    gasLimit: DecimalSchema.optional(),
    maxFeePerGas: DecimalSchema.optional(),
    maxPriorityFeePerGas: DecimalSchema.optional(),
  })
  .strict() satisfies z.ZodType<WorkflowGasOverrides>;
...
const WorkflowDeployStrategySchema = z.discriminatedUnion('kind', [
  ...
  z
    .object({
      ...
      salt: Hex32Schema,
      saltPerChain: z.record(ChainKeySchema, Hex32Schema).optional(),
      acknowledgeDeployed: AcknowledgementsSchema.optional(),
    })
    .strict(),
  z
    .object({
...
const WorkflowStepSchema = z
  .discriminatedUnion('kind', [
    z
      .object({
        ...
        argsPerChain: z.record(ChainKeySchema, z.record(z.string(), z.unknown())).optional(),
        value: DecimalSchema.optional(),
        valuePerChain: z.record(ChainKeySchema, DecimalSchema).optional(),
        gasOverrides: GasOverridesSchema.optional(),
        gasOverridesPerChain: z.record(ChainKeySchema, GasOverridesSchema.partial()).optional(),
        strategy: WorkflowDeployStrategySchema.optional(),
        libraries: z.record(z.string(), LibraryBindingSchema).optional(),
        librariesPerChain: z.record(ChainKeySchema, z.record(z.string(), LibraryBindingSchema)).optional(),
        wraps: z
          .object({
            stepId: z.string().min(1),
            contractTypePluginId: z.string().min(1),
          })
          .strict()
          .optional(),
    ...
    z
      .object({
        id: z.string().min(1),
        kind: z.literal('call'),
        target: CallTargetSchema,
        targetPerChain: z.record(ChainKeySchema, CallTargetSchema).optional(),
  ...
  .superRefine((step, ctx) => {
    if (step.kind === 'call' && (step.value !== undefined || step.valuePerChain !== undefined) && step.signature !== undefined && step.payable !== true)
      ctx.addIssue({
        code: 'custom',
        message: 'call value requires payable: true when signature is present',
        path: ['payable'],
      });
    ...
    const checkRefs = (value: unknown, path: (string | number)[]): void => {
      ...
      if (!Array.isArray(value) && '$book' in (value as Record<string, unknown>)) {
        if (!BookPointerSchema.safeParse(value).success)
          ctx.addIssue({
            code: 'custom',
            message: 'invalid strict $book value',
            path,
          });
        return;
      }
      if (!Array.isArray(value) && '$ref' in (value as Record<string, unknown>)) {
        ...
        if (!parsed.success)
          ctx.addIssue({
            code: 'custom',
            message: 'invalid strict $ref value',
            path,
          });
        return;
      }
    ...
    checkRefs(step.args, ['args']);
    checkRefs(step.argsPerChain, ['argsPerChain']);
...
export function makeWorkflowDocumentSchema(options: { allowFileUrls?: boolean } = {}): z.ZodType<WorkflowDocument> {
  const PinSchema = z
    .object({
      url: z
        .string()
        .min(1)
        .refine((value) => allowedPinUrl(value, options.allowFileUrls === true), 'pin URL must be credential-free https:// (or file:// in dev mode)'),
    ...
    .strict()
    .superRefine((pin, ctx) => {
      if (pin.ref !== undefined && pin.refKind === undefined)
        ctx.addIssue({
          code: 'custom',
          message: 'refKind is required when ref is present',
          path: ['refKind'],
        });
    });
  const RepoSourceSchema = z
    .object({
      ...
      sourcePath: z.string().refine(relativePath, 'sourcePath must be a relative path without dot segments'),
      contractName: z.string().min(1),
      artifactPath: z.string().refine(relativePath, 'artifactPath must be a relative path without dot segments'),
      artifactHash: z.string().regex(SHA256_HEX).optional(),
    })
    .strict() satisfies z.ZodType<RepoWorkflowSource>;
  const ContractTypeSourceSchema = z
    .object({
      id: z.string().min(1),
    ...
    .strict() satisfies z.ZodType<ContractTypeWorkflowSource>;
  ...
  return z
    .object({
      ...
      steps: z.array(WorkflowStepSchema).max(256), // deprecated: ignored, retained so existing documents validate
      defaultChains: z.array(z.number().int().positive()).max(128).optional(),
      requiredPlugins: z.array(RequiredPluginSchema).max(32),
      outputs: OutputsSchema,
    })
    .strict()
    .superRefine((doc, ctx) => {
      const duplicate = (values: string[]) => values.find((value, index) => values.indexOf(value) !== index);
      const source = duplicate(doc.sources.map((entry) => entry.id));
      if (source)
        ctx.addIssue({
          code: 'custom',
          message: `duplicate source id: ${source}`,
          path: ['sources'],
        });
      const step = duplicate(doc.steps.map((entry) => entry.id));
      if (step)
        ctx.addIssue({
          code: 'custom',
          message: `duplicate step id: ${step}`,
          path: ['steps'],
        });
      const plugin = duplicate(doc.requiredPlugins.map((entry) => entry.id));
      if (plugin)
        ctx.addIssue({
          code: 'custom',
          message: `duplicate plugin id: ${plugin}`,
          path: ['requiredPlugins'],
        });
      const hook = duplicate(doc.outputs.hooks);
      if (hook)
        ctx.addIssue({
          code: 'custom',
          message: `duplicate hook id: ${hook}`,
          path: ['outputs', 'hooks'],
        });
      if (doc.defaultChains && new Set(doc.defaultChains).size !== doc.defaultChains.length)
        ctx.addIssue({
          code: 'custom',
          message: 'default chains must be unique',
          path: ['defaultChains'],
        });
      ...
      const assertDeploy = (stepId: string, path: (string | number)[]) => {
        if (!deployIds.has(stepId))
          ctx.addIssue({
            code: 'custom',
            message: 'pointer, target, and library stepIds must name deploy steps',
            path,
          });
      };
      const visitRefs = (value: unknown, path: (string | number)[]) => {
        if (!value || typeof value !== 'object') return;
        if (!Array.isArray(value) && '$ref' in (value as Record<string, unknown>)) {
          const ref = value as WorkflowValueRef;
          if (ref.$ref?.kind === 'step') assertDeploy(ref.$ref.stepId, path);
          return;
        }
        Object.entries(value as Record<string, unknown>).forEach(([key, entry]) => visitRefs(entry, [...path, key]));
      };
      doc.steps.forEach((entry, index) => {
        if (entry.kind === 'deploy') {
          if (!contractIds.has(entry.contractId))
            ctx.addIssue({
              code: 'custom',
              message: 'step contractId must reference a source',
              path: ['steps', index, 'contractId'],
            });
          if (entry.wraps) assertDeploy(entry.wraps.stepId, ['steps', index, 'wraps', 'stepId']);
          visitRefs(entry.args, ['steps', index, 'args']);
          visitRefs(entry.argsPerChain, ['steps', index, 'argsPerChain']);
          Object.entries(entry.libraries ?? {}).forEach(([key, binding]) => {
            if (binding.kind === 'step') assertDeploy(binding.stepId, ['steps', index, 'libraries', key]);
          });
          Object.entries(entry.librariesPerChain ?? {}).forEach(([chain, bindings]) =>
            Object.entries(bindings).forEach(([key, binding]) => {
              if (binding.kind === 'step') assertDeploy(binding.stepId, ['steps', index, 'librariesPerChain', chain, key]);
            }),
          );
        } else {
          if (entry.target.kind === 'step') assertDeploy(entry.target.stepId, ['steps', index, 'target']);
          Object.entries(entry.targetPerChain ?? {}).forEach(([chain, target]) => {
            if (target.kind === 'step') assertDeploy(target.stepId, ['steps', index, 'targetPerChain', chain]);
          });
          visitRefs(entry.args, ['steps', index, 'args']);
          visitRefs(entry.argsPerChain, ['steps', index, 'argsPerChain']);
        }
      });
      if (jsonByteLength(doc) > MAX_DOCUMENT_BYTES)
        ctx.addIssue({
          code: 'custom',
          message: `workflow document exceeds ${MAX_DOCUMENT_BYTES} bytes`,
        });
...
export const InstalledWorkflowRecordSchema = z
  .object({
    ...
    installed: z
      .object({
        ...
        at: z.string().min(1),
        sources: z.array(InstalledSourceSnapshotSchema),
        plugins: z.array(
          z
            .object({
              id: z.string().min(1),
        ...
        stepsHash: z.string().regex(SHA256_HEX),
        hooksHash: z.string().regex(SHA256_HEX),
      })
      .strict()
      .optional(),
    lastAttempt: z
          ...
          .array(
            z
              .object({
                ...
                code: z.enum(['ARTIFACT_NOT_FOUND', 'FRAMEWORK_MISSING', 'LIFECYCLE_FAILED']).optional(),
...
export const InstalledWorkflowsFileSchema = z
  .object({
    schemaVersion: z.literal(1),
    records: z.array(z.unknown()),
    quarantinedAt: z.string().min(1).optional(),
  })
  .strict();
...
export const WorkflowSummarySchema = z
  .object({
    name: z.string().regex(WorkflowNamePattern),
    valid: z.boolean(),
    error: z.string().optional(),
    description: z.string().max(1024).optional(),
    sourceCount: z.number().int().nonnegative().optional(),
    stepCount: z.number().int().nonnegative().optional(),
    hooks: z.array(z.string().min(1)).max(16).optional(),
  })
  .strict() satisfies z.ZodType<WorkflowSummary>;
const WorkflowListResponseSchema = createApiResponseSchema<{
...
const WorkflowGetResponseSchema = createApiResponseSchema<{
  ...
  z
    .object({
      document: makeWorkflowDocumentSchema({ allowFileUrls: true }),
      raw: z.string(),
      docHash: z.string().regex(SHA256_HEX),
    })
    .strict(),
...
const WorkflowPutBodySchema = z
  .object({
    document: z.unknown(),
    baseDocHash: z.string().regex(SHA256_HEX).optional(),
  })
  .strict();
const WorkflowInstallBodySchema = z
  .object({
    repoPathOrUrl: z.string().min(1),
    name: z.string().regex(WorkflowNamePattern),
    expectedDocHash: z.string().regex(SHA256_HEX),
  })
  .strict();
const WorkflowApproveOriginsBodySchema = z.object({ origins: z.array(z.string().min(1)).min(1).max(64) }).strict();
const WorkflowApproveOriginsResponseSchema = createApiResponseSchema<{
  origins: string[];
}>('WorkflowApproveOriginsResponseSchema')(z.object({ origins: z.array(z.string()) }).strict());
...
const exactlyOnePromotionInput = (value: { plan?: unknown; runId?: string }, ctx: z.RefinementCtx) => {
  if ((value.plan === undefined) === (value.runId === undefined))
    ctx.addIssue({
      code: 'custom',
      message: 'exactly one of plan or runId is required',
    });
};
const PromotionPreviewRequestSchema = z
  .object({ mode: z.literal('preview'), ...PromotionSelectionShape })
  .strict()
  .superRefine(exactlyOnePromotionInput);
const PromotionApplyRequestSchema = z
  .object({
    mode: z.literal('apply'),
    previewId: z.string().min(1),
    ...PromotionSelectionShape,
    tagChoiceBySourceId: z.record(z.string().min(1), z.string().min(1)).optional(),
    overwrite: z.boolean().optional(),
    bookChoices: z
      .record(
        z.string().regex(/^[a-z0-9][a-z0-9-]{0,63}$/),
        z.discriminatedUnion('action', [
          z.object({ action: z.literal('keep-repo') }).strict(),
          z
            .object({
              action: z.literal('copy-under-new-name'),
    ...
    adoptRunIds: z.array(RunIdSchema).max(64).optional(),
  })
  .strict()
  .superRefine(exactlyOnePromotionInput)
  .superRefine((request, ctx) => {
    if (new Set(request.hooks).size !== request.hooks.length)
      ctx.addIssue({
        code: 'custom',
...
const PromotionSourcePreviewSchema = z
  .object({
    sourceId: z.string().min(1),
    origin: z.string(),
    commit: z.string(),
    tagChoices: z.array(z.string()),
    dirty: z.boolean(),
    error: z.string().optional(),
  })
  .strict();
const PromotionBookPreviewSchema = z
  .object({
    name: z.string().regex(/^[a-z0-9][a-z0-9-]{0,63}$/),
    entry: AddressBookEntrySchema,
    resolutions: z.record(ChainKeySchema, AddressSchema),
    source: z.enum(['local', 'repo']),
    bookHash: z.string().regex(SHA256_HEX),
    targetEntry: AddressBookEntrySchema.optional(),
    conflict: z.boolean(),
  })
  .strict();
  ...
  z.discriminatedUnion('mode', [
    z
      .object({
        ...
        previewId: z.string().min(1),
        sources: z.array(PromotionSourcePreviewSchema),
        referencedEntries: z.array(PromotionBookPreviewSchema),
        nameCollision: z.boolean(),
      })
      .strict(),
    z
      .object({
        ...
        docHash: z.string().regex(SHA256_HEX),
        warnings: z.array(z.string().min(1)).max(64).optional(),
      })
      .strict(),
  ]),
);
export const WorkflowCheckUpdatesRequestSchema = z
  .object({
    repoPathOrUrl: z.string().min(1),
    name: z.string().regex(WorkflowNamePattern),
  })
  .strict();
const WorkflowSourceUpdateSchema = z
  .object({
    sourceId: z.string().min(1),
    status: z.enum(['up-to-date', 'upgrade-available', 'tag-retargeted', 'tag-deleted', 'branch-moved', 'approval-required', 'contract-type-drift', 'error']),
    currentCommit: z.string().regex(COMMIT).optional(),
    latestCommit: z.string().regex(COMMIT).optional(),
    currentContentHash: z.string().regex(SHA256_HEX).optional(),
    latestContentHash: z.string().regex(SHA256_HEX).optional(),
...
const WorkflowPluginUpdateSchema = z
  .object({
    id: z.string().min(1),
    requiredVersion: z.string().min(1),
    status: z.enum(['installed', 'version-mismatch', 'missing']),
    installedVersion: z.string().optional(),
    updateAvailable: z.boolean(),
    update: PluginVersionInfoSchema.optional(),
  })
...
export const WorkflowCheckUpdatesResponseSchema = createApiResponseSchema<WorkflowCheckUpdatesData>('WorkflowCheckUpdatesResponseSchema')(
  z
    .object({
      docHash: z.string().regex(SHA256_HEX),
      sources: z.array(WorkflowSourceUpdateSchema),
      plugins: z.array(WorkflowPluginUpdateSchema),
    })
    .strict(),
);
const WorkflowSourceDetailSchema = z
  .object({
    id: z.string(),
    ...
    commit: z.string().regex(COMMIT).optional(),
  ...
  z
    .object({
      ...
      failedSources: z
        ...
        .optional(),
      atDocHash: z.string().regex(SHA256_HEX),
    })
    .strict(),
]) satisfies z.ZodType<WorkflowAttemptStatus>;
const WorkflowInstallDiffSchema = z
  .object({
    sourcesAdded: z.array(WorkflowSourceDetailSchema),
    sourcesRemoved: z.array(WorkflowSourceDetailSchema),
    sourcesRenamed: z.array(
      z
        .object({
          from: z.string(),
          to: z.string(),
          detail: WorkflowSourceDetailSchema,
        })
        .strict(),
    ),
    versionsChanged: z.array(
      z
        .object({
          detail: WorkflowSourceDetailSchema,
          from: z
    ...
    pluginsChanged: z.array(
      z
        .object({
          id: z.string(),
          kind: z.enum(['added', 'removed', 'version', 'source']),
    ...
    hooksChanged: z.boolean(),
    formattingOnly: z.boolean(),
  })
  .strict() satisfies z.ZodType<WorkflowInstallDiff>;
const WorkflowStatusEntrySchema = z
  .object({
    name: z.string(),
    valid: z.boolean(),
    error: z.string().optional(),
    docHash: z.string().regex(SHA256_HEX).optional(),
    installedDocHash: z.string().regex(SHA256_HEX).optional(),
    installState: z.enum(['not-installed', 'out-of-sync', 'ready']).optional(),
    attempt: WorkflowAttemptStatusSchema.optional(),
    diff: WorkflowInstallDiffSchema.optional(),
    sources: z.array(WorkflowSourceDetailSchema).optional(),
    plugins: z.array(WorkflowPluginReadinessSchema).optional(),
  })
  .strict() satisfies z.ZodType<WorkflowStatusEntry>;
const WorkflowStatusResponseSchema = createApiResponseSchema<{
  workflows: WorkflowStatusEntry[];
}>('WorkflowStatusResponseSchema')(z.object({ workflows: z.array(WorkflowStatusEntrySchema) }).strict());

export const workflowRoutes = {
  listWorkflows: {
    method: 'GET' as const,
    path: `${V1_BASE_PATH}/repos/workflows`,
    schema: {
      tags: ['repos'],
      querystring: WorkflowPathQuerySchema,
      response: { 200: WorkflowListResponseSchema },
    },
  },
  getWorkflowsStatus: {
    method: 'GET' as const,
    path: `${V1_BASE_PATH}/repos/workflows/status`,
    schema: {
      tags: ['repos'],
      querystring: WorkflowPathQuerySchema,
      response: { 200: WorkflowStatusResponseSchema },
    },
  },
  getWorkflow: {
    method: 'GET' as const,
    path: `${V1_BASE_PATH}/repos/workflows/:name`,
    schema: {
      tags: ['repos'],
      params: WorkflowNameParamsSchema,
      querystring: WorkflowPathQuerySchema,
      response: { 200: WorkflowGetResponseSchema },
    },
  },
  putWorkflow: {
    method: 'PUT' as const,
    ...
    schema: {
      ...
      params: WorkflowNameParamsSchema,
      querystring: WorkflowPathQuerySchema,
      body: WorkflowPutBodySchema,
      response: { 200: WorkflowPutResponseSchema },
    },
  },
  installWorkflow: {
    method: 'POST' as const,
    path: `${V1_BASE_PATH}/workflows/install`,
    schema: {
      tags: ['workflows'],
      body: WorkflowInstallBodySchema,
      response: { 200: JobStartedResponseSchema },
    },
  },
  approveWorkflowOrigins: {
    method: 'POST' as const,
    path: `${V1_BASE_PATH}/workflows/approve-origins`,
    schema: {
      tags: ['workflows'],
      body: WorkflowApproveOriginsBodySchema,
      response: { 200: WorkflowApproveOriginsResponseSchema },
    },
  },
  promoteWorkflow: {
    method: 'POST' as const,
    path: `${V1_BASE_PATH}/workflows/promote`,
    schema: {
      tags: ['workflows'],
      body: WorkflowPromoteRequestSchema,
      response: { 200: WorkflowPromoteResponseSchema },
    },
  },
  checkWorkflowUpdates: {
    method: 'POST' as const,
    path: `${V1_BASE_PATH}/workflows/check-updates`,
    schema: {
      tags: ['workflows'],
      body: WorkflowCheckUpdatesRequestSchema,
      response: { 200: WorkflowCheckUpdatesResponseSchema },
    },
  },
} as const;
