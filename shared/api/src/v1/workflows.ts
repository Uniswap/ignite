...
export type WorkflowDeployStrategy =
  ...
  | { kind: 'plugin'; pluginId: string; params?: Record<string, unknown>; producedBy?: { stepId: string; outputIndex: number }; salt?: string; saltPerChain?: Record<string, string>; prepared?: Record<string, { initcodeHash: string; predictedAddress: string }>; acknowledgeDeployed?: Record<string, { predictedAddress: string; initcodeHash: string }> };
...
export type WorkflowPluginReadiness =
  ...
  | { id: string; status: 'wrong-type'; installedVersion: string };
export interface WorkflowInstallResult { sources: WorkflowSourceReadiness[]; plugins: WorkflowPluginReadiness[] }
...
const WorkflowDeployStrategySchema = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('plugin'), pluginId: z.string().min(1), params: z.record(z.string(), z.unknown()).optional(), producedBy: z.object({ stepId: z.string().min(1), outputIndex: z.number().int().nonnegative() }).strict().optional(), salt: Hex32Schema.optional(), saltPerChain: z.record(ChainKeySchema, Hex32Schema).optional(), prepared: z.record(ChainKeySchema, z.object({ initcodeHash: Hex32Schema, predictedAddress: AddressSchema }).strict()).optional(), acknowledgeDeployed: AcknowledgementsSchema.optional() }).strict()
    // Same two-state rule as the plan schema: a produced product never
    // carries CREATE2 commitments or acknowledgements.
...
export function makeWorkflowDocumentSchema(options: { allowFileUrls?: boolean } = {}): z.ZodType<WorkflowDocument> {
  ...
  return z.object({ schemaVersion: z.literal(1), description: z.string().max(1024).optional(), sources: z.array(SourceSchema).max(64), steps: z.array(WorkflowStepSchema).max(256), // deprecated: ignored, retained so existing documents validate
    ...
    // Produced mode is checked here, not only in the plan schema: a workflow is
    // a hand-editable committed document, so a typo'd producedBy or a
    // transaction-only field on a product must fail at install with a document
    // path rather than later as a plan-index error on a generated plan.
    const callIndexById = new Map(doc.steps.flatMap((entry, index) => entry.kind === 'call' ? [[entry.id, index] as const] : []));
    const producerIds = new Set(doc.steps.flatMap((entry) => entry.kind === 'deploy' && entry.strategy?.kind === 'plugin' && entry.strategy.producedBy ? [entry.strategy.producedBy.stepId] : []));
    doc.steps.forEach((entry, index) => { if (entry.kind === 'deploy') { if (!contractIds.has(entry.contractId)) ctx.addIssue({ code: 'custom', message: 'step contractId must reference a source', path: ['steps', index, 'contractId'] }); if (entry.wraps) assertDeploy(entry.wraps.stepId, ['steps', index, 'wraps', 'stepId']); visitRefs(entry.args, ['steps', index, 'args']); visitRefs(entry.argsPerChain, ['steps', index, 'argsPerChain']); Object.entries(entry.libraries ?? {}).forEach(([key, binding]) => { if (binding.kind === 'step') assertDeploy(binding.stepId, ['steps', index, 'libraries', key]); }); Object.entries(entry.librariesPerChain ?? {}).forEach(([chain, bindings]) => Object.entries(bindings).forEach(([key, binding]) => { if (binding.kind === 'step') assertDeploy(binding.stepId, ['steps', index, 'librariesPerChain', chain, key]); })); if (entry.strategy?.kind === 'plugin' && entry.strategy.producedBy) { const producerIndex = callIndexById.get(entry.strategy.producedBy.stepId); if (producerIndex === undefined) ctx.addIssue({ code: 'custom', message: 'producedBy must reference a call step', path: ['steps', index, 'strategy', 'producedBy', 'stepId'] }); else if (producerIndex >= index) ctx.addIssue({ code: 'custom', message: 'producedBy must reference an earlier call step', path: ['steps', index, 'strategy', 'producedBy', 'stepId'] }); for (const field of ['value', 'valuePerChain', 'gasOverrides', 'gasOverridesPerChain', 'libraries', 'librariesPerChain'] as const) if (entry[field] !== undefined) ctx.addIssue({ code: 'custom', message: `${field} is not valid on a produced deployment step`, path: ['steps', index, field] }); } } else { if (entry.target.kind === 'step') assertDeploy(entry.target.stepId, ['steps', index, 'target']); Object.entries(entry.targetPerChain ?? {}).forEach(([chain, target]) => { if (target.kind === 'step') assertDeploy(target.stepId, ['steps', index, 'targetPerChain', chain]); }); visitRefs(entry.args, ['steps', index, 'args']); visitRefs(entry.argsPerChain, ['steps', index, 'argsPerChain']); if (entry.abiContractId !== undefined && !contractIds.has(entry.abiContractId)) ctx.addIssue({ code: 'custom', message: 'step abiContractId must reference a source', path: ['steps', index, 'abiContractId'] }); if (producerIds.has(entry.id) && entry.abiContractId === undefined) ctx.addIssue({ code: 'custom', message: 'a producer call requires abiContractId', path: ['steps', index, 'abiContractId'] }); } });
