...
import type { DeploymentPlan, DeployStep, PauseReason, ResolveAction, ResolveLaneRequest, RunEvent, RunRecord, Hex } from '@ignite/api';
import { allowedActions, CREATE2_PROXY_ADDRESS } from '@ignite/api';
import { DeployEngine, type DeployEngineDeps } from '../../deployments/DeployEngine.js';
...
function validated(plan: DeploymentPlan) {
  ...
  return {
    report: {
      chains: Object.fromEntries(plan.chains.map((chainId) => [String(chainId), checklist])),
    },
    ...
    rpcBindings: Object.fromEntries(plan.chains.map((chainId) => [String(chainId), { endpointId: 'rpc', label: 'Anvil', urlFingerprint: HASH }])),
  };
}

async function eventually(predicate: () => Promise<boolean> | boolean, label = 'condition'): Promise<void> {
  for (let i = 0; i < 200; i++) {
...
describe('DeployEngine', () => {
  ...
  function makeEngine(
    ...
    const store = (deps?.runStore as RunStore) ?? new RunStore({ baseDir: home });
  ...
  async function launchDefault(harness: Harness, plan = makePlan(), key = crypto.randomUUID()) {
    return harness.engine.launch({
      profileId: 'p1',
      plan,
      rpcSelection: Object.fromEntries(plan.chains.map((chainId) => [String(chainId), 'rpc'])),
      idempotencyKey: key,
    });
  }
  ...
  async function seedPausedRun(harness: Harness, opts: { reason: PauseReason; submitted: boolean; chainId?: number }) {
    ...
    await eventually(async () => (await gated.engine.get('p1', run.id))?.lanes[String(plan.chains[0])]?.status === 'running', 'lane running');
    ...
    await harness.store.mutate('p1', run.id, (current) => {
      ...
      lane.status = 'paused';
      lane.steps[0].status = opts.reason === 'revert' ? 'failed' : 'broadcasting';
  ...
  it('runs lanes in parallel with strict in-lane sequencing and completes the run', async () => {
    ...
    const harness = makeEngine({
      executeTx: async (args) => {
        const index = order.filter((entry) => entry.chainId === args.chainId).length;
    ...
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['2']?.status === 'completed', 'chain 2 completed');
    const midway = (await harness.engine.get('p1', run.id))!;
    expect(midway.lanes['1'].currentStepIndex).toBe(0);

    gate.release();
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'completed', 'run completed');
    ...
    for (const lane of Object.values(done.lanes)) {
      ...
      expect(lane.steps.every((step) => step.address === RECEIPT.contractAddress)).toBe(true);
    }
    // In-lane sequencing: step 2 of a chain never starts before step 1.
    for (const chainId of [1, 2]) {
      const perChain = order.filter((entry) => entry.chainId === chainId).map((entry) => entry.index);
  ...
  it('retries a failed plugin prepare and re-mines', async () => {
    ...
    const prepare = vi
      ...
      .mockImplementation(async (_pluginId: string, input: { initcode: `0x${string}` }) => ({
        salt,
        predictedAddress: predictCreate2Address(salt, initcodeHashOf(input.initcode)),
        notes: [],
      }));
    const harness = makeEngine({
      validate: async () => dynamicValidation(plan),
    ...
    await harness.engine.resolveLane('p1', run.id, 1, {
      action: 'retry',
      attemptId: current.lanes['1'].pause!.attemptId,
      ...
      steps: [
        {
          ...
          kind: 'deploy',
          contractId: 'c1',
          strategy: { kind: 'create2', salt: `0x${'48'.repeat(32)}` },
        },
      ],
    });
  ...
  it('launches through non-blocking provisional degradation and pauses at JIT execution', async () => {
    ...
    const harness = makeEngine({
      validate: async () => {
        const result = dynamicValidation(plan);
        const chain = result.report.chains['1'] as Record<
          string,
          {
            ok: boolean;
            blocking: boolean;
        ...
        chain.create2 = {
          ok: true,
          blocking: false,
          message: 'provisional unavailable',
          details: {
            provisionalSteps: [{ stepId: 'step-2', degraded: 'bad flags' }],
          },
        };
        chain.estimation = {
          ok: false,
          blocking: false,
          message: 'estimation unavailable',
        };
        chain.simulation = {
          ok: false,
          blocking: false,
  ...
  it('accepts a dynamic collision without a plan salt and records acknowledgement provenance', async () => {
    ...
    expect(step).toMatchObject({
      status: 'skipped',
      address: step.predictedAddress,
      salt,
    });
    const strategy = (current.plan.steps[1] as DeployStep).strategy;
    expect(strategy && strategy.kind !== 'create' ? strategy.acknowledgeDeployed?.['1'] : undefined).toEqual({
      predictedAddress: step.predictedAddress,
      initcodeHash: expect.stringMatching(/^0x[0-9a-f]{64}$/),
    });
  });
      ...
      steps: [
        {
          ...
          kind: 'deploy',
          contractId: 'c1',
          strategy: { kind: 'create2', salt },
        },
      ],
    });
    const initcodeHash = initcodeHashOf('0x6000');
    const predictedAddress = predictCreate2Address(salt, initcodeHash);
    const harness = makeEngine({
      validate: async () => ({
        ...validated(plan),
        predicted: {
          '1': { 'step-1': { salt, initcodeHash, predictedAddress } },
        },
      }),
      getCode: async () => '0x01',
    });
  ...
  it('skips an acknowledged dynamic collision idempotently', async () => {
    ...
    strategy.acknowledgeDeployed = {
      '1': { predictedAddress, initcodeHash: initcodeHashOf(initcode) },
    };
    const harness = makeEngine({
      validate: async () => dynamicValidation(plan),
      deploymentTypes: {
        prepare: vi.fn(async () => ({ salt, predictedAddress, notes: [] })),
        list: async () => [],
        validate: vi.fn(),
      },
      getCode: async () => '0x01',
    ...
    expect(current.lanes['1'].steps[1]).toMatchObject({
      status: 'skipped',
      address: predictedAddress,
      attempts: [{ resolution: 'accept-deployed', endedAt: expect.any(String) }],
    });
    expect(harness.executed).toHaveLength(1);
  });
  ...
  it('recovers a real attempt when restarted after atomic JIT persistence before send', async () => {
    ...
    const second = makeEngine({
      ...
      deploymentTypes: { prepare, list: async () => [], validate: vi.fn() },
      getCode: async () => (++recoveryReads === 1 ? '0x' : '0x01'),
    });
    ...
    expect(recovered.lanes['1'].pause).toMatchObject({
      reason: 'interrupted',
      attemptId: step.attempts[0].id,
    });
    expect(step).toMatchObject({
      salt,
      notes: ['durable'],
      attempts: [{ expected: { pointers: { 'args.owner': RECEIPT.contractAddress } } }],
    });
  ...
  it('clears seeded static facts when an edit flips the step dynamic', async () => {
    const salt = `0x${'53'.repeat(32)}` as const;
    const gate = createGate();
    const plan = makePlan({
      chains: [1],
      steps: [
        { id: 'step-1', kind: 'deploy', contractId: 'c1' },
        {
          ...
          contractId: 'c2',
          args: { owner: ADDRESS },
          strategy: { kind: 'plugin', pluginId: 'hook', salt },
        },
      ],
    });
    const initial = dynamicValidation(plan);
    const deploy = plan.steps[1] as DeployStep;
    const initcode = buildInitcode(deploy, initial.frozen.c2, 1, () => RECEIPT.contractAddress);
    const prediction = {
      salt,
      initcodeHash: initcodeHashOf(initcode),
      predictedAddress: predictCreate2Address(salt, initcodeHashOf(initcode)),
    };
    const strategy = deploy.strategy;
    if (!strategy || strategy.kind !== 'plugin') throw new Error('bad fixture');
    strategy.prepared = {
      '1': {
        initcodeHash: prediction.initcodeHash,
        predictedAddress: prediction.predictedAddress,
      },
    };
    const harness = makeEngine({
      validate: async () => ({
        ...dynamicValidation(plan),
        predicted: { '1': { 'step-2': prediction } },
      }),
      deploymentTypes: {
        prepare: vi.fn(async (_id, input) => {
          await gate.promise;
          return {
            salt,
            predictedAddress: predictCreate2Address(salt, initcodeHashOf(input.initcode)),
    ...
    await harness.store.mutate('p1', run.id, (current) => {
      current.lanes['1'].pause!.reason = 'estimation';
    });
    const edited = await harness.engine.resolveLane('p1', run.id, 1, {
      action: 'edit',
      attemptId: paused.lanes['1'].pause!.attemptId,
      commandId: crypto.randomUUID(),
      edits: {
        argsByStep: {
          'step-2': { owner: { $ref: { kind: 'step', stepId: 'step-1' } } },
        },
      },
    });
  ...
  it('clears JIT salt and notes and reseeds prediction when an edit flips dynamic to static', async () => {
    const planSalt = `0x${'54'.repeat(32)}` as const;
    const jitSalt = `0x${'55'.repeat(32)}` as const;
    ...
    strategy.prepared = {
      '1': {
        initcodeHash: initcodeHashOf(staticInitcode),
        predictedAddress: staticPrediction,
      },
    };
    const harness = makeEngine({
      validate: async () => dynamicValidation(plan),
      deploymentTypes: {
        prepare: vi.fn(async (_id, input) => ({
          salt: jitSalt,
          predictedAddress: predictCreate2Address(jitSalt, initcodeHashOf(input.initcode)),
          notes: ['old JIT'],
        })),
        list: async () => [],
        validate: vi.fn(),
    ...
    const edited = await harness.engine.resolveLane('p1', run.id, 1, {
      ...
      attemptId: paused.lanes['1'].pause!.attemptId,
      commandId: crypto.randomUUID(),
      edits: { argsByStep: { 'step-2': { owner: ADDRESS } } },
    });
  ...
  it('atomically adds the durable hook outbox on the first terminal transition and dispatches only once', async () => {
    ...
    const runStore = {
      ...
      findByIdempotencyKey: store.findByIdempotencyKey.bind(store),
      recoverStartup: store.recoverStartup.bind(store),
      listAllRuns: store.listAllRuns.bind(store),
      mutate: (profileId: string, runId: string, fn: (run: RunRecord) => void) =>
        store.mutate(profileId, runId, (draft) => {
          fn(draft);
    ...
    const harness = makeEngine({
      runStore,
      deploymentHooks: { dispatch, reconcileStartup: async () => undefined },
    });
    const plan = makePlan({ chains: [1] });
    const workflow = {
      repoPathOrUrl: '/workflow',
      name: 'release',
      hooks: ['chronicles'],
      docHash: HASH,
    };
  ...
  it('enforces exactly the shared verb table for every pause context', async () => {
    const ALL_ACTIONS: ResolveAction[] = ['retry', 'edit', 'skip', 'abort-lane', 'recheck', 'confirm-hash', 'mark-not-sent', 'replace', 'keep-waiting'];
    const REASONS: PauseReason[] = ['revert', 'estimation', 'balance', 'broadcast', 'rpc', 'signer-rejected', 'needs-browser', 'receipt-timeout', 'interrupted', 'needs-review', 'write-failure', 'signer-mismatch', 'rpc-binding-changed'];
    for (const capability of ['sign-only', 'sign-and-send'] as const) {
      for (const reason of REASONS) {
        for (const submitted of [false, true]) {
          ...
          for (const action of ALL_ACTIONS.filter((entry) => !allowed.includes(entry))) {
            const cmd = {
              action,
              attemptId,
              commandId: crypto.randomUUID(),
            ...
            await expect(harness.engine.resolveLane('p1', run.id, chainId, cmd), `${reason}/${capability}/submitted=${submitted} must forbid ${action}`).rejects.toMatchObject({ code: ErrorCodes.ILLEGAL_RESOLVE });
          }
  ...
  it('rejects address book drift at launch and a re-review freezes the new address', async () => {
    ...
    const run = await harness.engine.launch({
      ...
      plan: sourcePlan,
      rpcSelection: { '1': 'rpc' },
      idempotencyKey: 'fresh-book',
      expectedBookHash: { local: secondHash },
    });
    expect(run.plan.steps[0]).toMatchObject({ args: { owner: secondAddress } });
    expect(run.bookResolutions?.['1']?.[0]).toMatchObject({
      address: secondAddress,
      bookHash: secondHash,
    });
  });

  it('retries from the frozen literal plan without re-resolving a mutated book', async () => {
    const frozenAddress = '0x1111111111111111111111111111111111111111' as const;
    const changedAddress = '0x2222222222222222222222222222222222222222' as const;
    let bookAddress: Hex = frozenAddress;
    let sends = 0;
    ...
    const validate = vi.fn(async () => {
      const resolved = globalThis.structuredClone(sourcePlan);
      resolved.steps[0]!.args = { owner: bookAddress };
      const result = validated(resolved);
      (result.frozen.c1 as { abi: unknown }).abi = [{ type: 'constructor', inputs: [{ name: 'owner', type: 'address' }] }];
      return { ...result, resolvedPlan: resolved, bookHashes: { local: HASH } };
    });
    const harness = makeEngine({
      validate,
      executeTx: async (args) => {
    ...
    const run = await harness.engine.launch({
      ...
      plan: sourcePlan,
      rpcSelection: { '1': 'rpc' },
      idempotencyKey: 'frozen-retry',
      expectedBookHash: { local: HASH },
    });
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused');
    bookAddress = changedAddress;
    const paused = (await harness.engine.get('p1', run.id))!;
    await harness.engine.resolveLane('p1', run.id, 1, {
      action: 'retry',
      attemptId: paused.lanes['1'].pause!.attemptId,
    ...
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'completed');
    expect(payloads).toHaveLength(2);
    expect(payloads[1]).toBe(payloads[0]);
    expect(payloads[1].toLowerCase()).toContain(frozenAddress.slice(2));
    expect(payloads[1].toLowerCase()).not.toContain(changedAddress.slice(2));
  ...
  it('persists the server-resolved workflow binding and passes its document to validation', async () => {
    ...
    expect(validate).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        workflow: { binding: workflow, document: workflowDocument },
      })
    );
  });
  ...
  it('pauses with write-failure and never broadcasts when the intent write fails', async () => {
      ...
      get(target, prop, receiver) {
        ...
        return (profileId: string, runId: string, fn: (run: never) => void) =>
          target.mutate(profileId, runId, (run) => {
            ...
            if (failNextBroadcastWrite && lane?.steps.some((step) => step.status === 'broadcasting')) {
    ...
    await eventually(async () => (await store.get('p1', run.id))?.lanes['1']?.status === 'paused', 'lane paused');
  ...
  it('resume reconciles a persisted sign-only hash: mined confirms, unknown rebroadcasts the raw tx', async () => {
    ...
    await eventually(async () => (await minedHarness.engine.get('p1', mined.run.id))?.lanes['1'].steps[0].status === 'confirmed', 'reconciled');
    ...
    await eventually(async () => (await unknownHarness.engine.get('p1', unknown.run.id))?.lanes['1'].steps[0].status === 'confirmed', 'rebroadcast reconciled');
  });
  ...
  it('confirm-hash verifies the receipt and continues; mark-not-sent enables a fresh retry', async () => {
    ...
    await eventually(async () => (await retryHarness.engine.get('p1', second.run.id))?.status === 'completed', 'fresh retry completed');
    const attempts = (await retryHarness.engine.get('p1', second.run.id))!.lanes['1'].steps[0].attempts;
  ...
  it('replace pins the nonce and applies the edited gas to the next attempt', async () => {
    ...
    await eventually(async () => harness.executed.length > 0, 'replacement executed');
  ...
  it('keep-waiting resumes receipt polling for the existing transaction', async () => {
    ...
    await eventually(async () => (await harness.engine.get('p1', seeded.run.id))?.status === 'completed', 'receipt polling completed');
    expect((await harness.engine.get('p1', seeded.run.id))!.lanes['1'].steps[0].attempts[0].txStatus).toBe('success');
  });
  ...
  it('keep-waiting pauses instead of re-executing when RPC resolution fails', async () => {
    ...
    await eventually(async () => (await harness.engine.get('p1', seeded.run.id))?.lanes['1'].status === 'paused', 'RPC failure paused');
    expect(harness.executed).toHaveLength(0);
    expect((await harness.engine.get('p1', seeded.run.id))!.lanes['1'].pause?.reason).toBe('rpc');
  });
  ...
  it('applies arg edits to the paused and later steps only, recording them on the attempt', async () => {
    ...
    const attempt = run.lanes['1'].steps[0].attempts.find((entry) => entry.id === seeded.attemptId)!;
  ...
  it('accepts a gas-only edit while a future dynamic deployment awaits the current receipt', async () => {
    ...
    const edited = await harness.engine.resolveLane('p1', seeded.run.id, seeded.chainId, {
      ...
      attemptId: seeded.attemptId,
      commandId: crypto.randomUUID(),
      edits: { gas: { gasLimit: '400000' } },
    });
    expect(edited.plan.steps[0].gasOverridesPerChain?.['1']).toEqual({
      gasLimit: '400000',
    });
  });
  ...
  it('records unresolvedTx when skipping a step whose tx may be in flight', async () => {
    ...
    await eventually(async () => (await harness.engine.get('p1', seeded.run.id))?.status === 'completed', 'lane continued');
    const step = (await harness.engine.get('p1', seeded.run.id))!.lanes['1'].steps[0];
  ...
  it('abort waits for the in-flight step, then aborts cleanly with status aborted', async () => {
    ...
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].steps[0].status === 'broadcasting', 'in flight');
    const abortPromise = harness.engine.abort('p1', run.id);
    gate.release();
    await abortPromise;
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'aborted', 'aborted at safe point');
    const done = (await harness.engine.get('p1', run.id))!;
  ...
  it('sanitizes secret-bearing errors before they reach attempts, pauses, and events', async () => {
    ...
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused', 'paused');
    const paused = (await harness.engine.get('p1', run.id))!;
  ...
  it('emits monotonic per-run events, replays within the epoch, and returns nothing across epochs', async () => {
    ...
    harness.engine.subscribe((_runId, event) => events.push(event));
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'completed', 'completed');
  ...
  it('pauses with reason balance on an insufficient-funds failure', async () => {
    const harness = makeEngine({
      executeTx: async () => {
        throw new IgniteError('Account balance is too low for this transaction', ErrorCodes.INSUFFICIENT_FUNDS);
      },
    });
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused', 'paused');
    expect((await harness.engine.get('p1', run.id))!.lanes['1'].pause?.reason).toBe('balance');
  });
  ...
  it('creates one immutable attempt per retry', async () => {
    ...
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused', 'paused');
    const first = (await harness.engine.get('p1', run.id))!.lanes['1'].steps[0].attempts[0];
    await harness.engine.resolveLane('p1', run.id, 1, {
      action: 'retry',
      attemptId: first.id,
      commandId: crypto.randomUUID(),
    ...
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'completed', 'completed');
    const attempts = (await harness.engine.get('p1', run.id))!.lanes['1'].steps[0].attempts;
  ...
  it('run-wide abort takes precedence after terminating a failed paused lane', async () => {
    ...
    expect(aborted.lanes['1'].steps[0].attempts[0].resolution).toBe('abort-run');
  });
  ...
  it('rejects a launch whose run-level validation has a blocking failure', async () => {
    const harness = makeEngine({
      validate: async (plan) => {
        const result = validated(plan);
        (result.report as typeof result.report & { run: object }).run = {
          workflow: { ok: false, blocking: true, message: 'binding failed' },
        };
        return result;
      },
    });
...
function holdUntilAborted(gate: Promise<void>, signal: AbortSignal): Promise<never> {
...
describe('final-review regressions', () => {
      ...
      contracts: [
        {
          ...
          sourcePath: 'Implementation.sol',
        },
        {
          id: 'proxy',
          origin: 'contract-type',
          contractName: 'TransparentUpgradeableProxy',
          pluginId: 'transparent',
          artifactKey: 'proxy',
          versionLabel: 'v1',
      ...
      steps: [
        { id: 'implementation', kind: 'deploy', contractId: 'implementation' },
        {
          ...
          wraps: {
            ...
            contractTypePluginId: 'transparent',
          },
          args: {
            implementation: {
              $ref: { kind: 'step', stepId: 'implementation' },
            },
      ...
      transparent: {
        ...
        descriptor: {
          ...
          capture: [
            ...
            {
              slot: `0x${'b5'.repeat(32)}`,
              record: 'admin',
              derivedCreate: { nonce: 1 },
              expectCodeOf: 'admin',
              verifyAs: 'admin',
              constructorArgs: ['initialOwner'],
              assertCalls: [{ call: 'owner()', on: 'admin', expectParam: 'initialOwner' }],
            },
          ],
        },
        artifacts: {
          proxy: {
            ...
            sourceIdentifier: 'Proxy.sol:Proxy',
          },
          admin: {
            abi: [
              {
            ...
            standardJsonInput: {
              language: 'Solidity',
              sources: { 'Admin.sol': { content: 'contract Admin {}' } },
              settings: {},
            },
            sourceIdentifier: 'Admin.sol:Admin',
          },
        },
      },
    };
  ...
  function makeCaptureEngine(deps: Partial<DeployEngineDeps>) {
    ...
    const engine = new DeployEngine({
      runStore: store,
      validate: async (plan) => transparentCaptureValidation(plan),
      resolveRpcUrl: async () => ({
        url: 'http://rpc.local',
        fingerprint: HASH,
      }),
      ...
      resolveAccount: async () => ({
        account: { id: 'a', address: ADDRESS, capability: 'sign-only' },
      }),
      chainMetadata: async () => ({
        name: 'Anvil',
        nativeCurrency: { name: 'Ether', symbol: 'ETH', decimals: 18 },
  ...
  it('F6: a failure-skipped step never resolves pointers via its prediction', async () => {
    ...
    const withCtor = (p: DeploymentPlan) => {
      ...
      (base.frozen as Record<string, { abi: unknown }>).c2.abi = [{ type: 'constructor', inputs: [{ name: 'owner', type: 'address' }] }];
      return base;
    };
    const engine = new DeployEngine({
      runStore: store,
    ...
    try {
      ...
      await store.mutate('p1', run.id, (current) => {
        current.lanes['1'].steps[0].predictedAddress = '0x00000000000000000000000000000000000000aa';
      });
      const paused = await store.get('p1', run.id);
      ...
      expect(final?.lanes['1'].pause?.details).toMatchObject({
        stepId: 'step-1',
      });
    } finally {
  ...
  it('F8: a successful plain-create receipt without contractAddress pauses needs-review', async () => {
    ...
    const plan = makePlan({
      chains: [1],
      steps: [{ id: 'step-1', kind: 'deploy', contractId: 'c1' }],
    });
    const engine = new DeployEngine({
      runStore: store,
    ...
    const harness = makeCaptureEngine({
      validate: async (candidate) => {
        ...
        type.descriptor.capture = [
          ...
          {
            slot: `0x${'c1'.repeat(32)}`,
            expectCodeOf: 'proxy',
            assertCalls: [{ call: 'owner()', on: 'admin', expectParam: 'initialOwner' }],
          },
        ];
        type.artifacts.proxy.abi = [
          {
            type: 'function',
            name: 'owner',
            inputs: [],
  ...
  it('rejects a lane edit that swaps wrapper calldata to unacknowledged empty data', async () => {
    const plan = transparentCapturePlan();
    (plan.steps[1] as DeployStep).args!._data = {
      $encode: { contractId: 'implementation', fn: 'initialize()' },
    };
    let sends = 0;
    const harness = makeCaptureEngine({
      validate: async (candidate) => {
        const result = transparentCaptureValidation(candidate);
        result.frozen.implementation.abi = [
          {
            type: 'function',
            name: 'initialize',
            inputs: [],
    ...
    await expect(
      ...
      code: ErrorCodes.ILLEGAL_RESOLVE,
      details: { contractTypeCode: 'UNINITIALIZED_PROXY_ACK_REQUIRED' },
    });
  });

  it.each(['accept-deployed', 'created-code-missing'] as const)('runs wrapper capture through %s completion paths', async (path) => {
    const plan = transparentCapturePlan();
    (plan.steps[1] as DeployStep).strategy = {
      kind: 'plugin',
      pluginId: 'hook',
    };
    ...
    const paused = (await harness.engine.get('p1', run.id))!;
    await harness.engine.resolveLane('p1', run.id, 1, {
      action: path === 'accept-deployed' ? 'accept-deployed' : 'recheck',
      attemptId: paused.lanes['1'].pause!.attemptId,
      commandId: crypto.randomUUID(),
    } as any);
  ...
  it('rejects a capture slot with nonzero high bits even when its low address matches', async () => {
    ...
    const proxy = RECEIPT.contractAddress;
    const harness = makeCaptureEngine({
      getStorageAt: async (_url, _address, slot) => (slot === `0x${'36'.repeat(32)}` ? (`0x${'ff'.repeat(12)}${proxy.slice(2)}` as `0x${string}`) : (`0x${'0'.repeat(24)}${getContractAddress({ from: proxy, nonce: 1n }).slice(2)}` as `0x${string}`)),
      getCode: async () => '0x6001',
    });
    const run = await launchCapture(harness.engine, plan);
  ...
  it('retries capture reads after an RPC pause when resumed', async () => {
    ...
    const harness = makeCaptureEngine({
      validate: async (candidate) => transparentCaptureValidation(candidate),
      getStorageAt: async (_url, _address, slot) => {
        reads += 1;
        if (fail) throw new Error('temporary RPC outage');
        return slot === `0x${'36'.repeat(32)}` ? word(proxy) : word(admin);
      },
      getCode: async () => '0x6001',
      call: async () =>
        encodeFunctionResult({
  ...
  it('reconciles an rpc-paused sign-and-send capture with a hash but no raw transaction', async () => {
    ...
    const harness = makeCaptureEngine({
      resolveAccount: async () => ({
        account: { id: 'a', address: ADDRESS, capability: 'sign-and-send' },
      }),
      executeTx: async (args) => {
        await args.onPhase?.('broadcasting', {
          tx: { nonce: 1 },
