...
import { LocalWorkflowStore } from './LocalWorkflowStore.js';
...
type LegacyRepoTarget = { repoPathOrUrl: string; name: string };
type LegacyPreviewRequest = Omit<PreviewRequest, 'target'> & { target: LegacyRepoTarget };
type LegacyApplyRequest = Omit<ApplyRequest, 'target'> & { target: LegacyRepoTarget };
...
export interface WorkflowPromotionServiceDeps {
  ...
  localWorkflows: Pick<LocalWorkflowStore, 'read' | 'write'>;
}
...
export class WorkflowPromotionService {
  ...
  async promote(request: LegacyPreviewRequest, profileId: string): Promise<PreviewData>;
  async promote(request: LegacyApplyRequest, profileId: string): Promise<ApplyData>;
  async promote(request: WorkflowPromoteRequest | LegacyPreviewRequest | LegacyApplyRequest, profileId: string): Promise<WorkflowPromoteData> {
    const normalized = ('kind' in request.target ? request : { ...request, target: { kind: 'repo', ...request.target } }) as WorkflowPromoteRequest;
    this.validateTarget(normalized.target);
    return normalized.mode === 'preview' ? this.preview(normalized, profileId) : this.apply(normalized, profileId);
  }
  ...
  private async preview(request: PreviewRequest, profileId: string): Promise<PreviewData> {
    ...
    if (document) {
      for (const source of document.sources) {
        if (source.origin === 'contract-type') sources.push({ sourceId: source.id, origin: 'contract-type', commit: source.contentHash, tagChoices: [], dirty: false });
        else sources.push({ sourceId: source.id, origin: source.repo.url, commit: source.repo.commit, tagChoices: source.repo.refKind === 'tag' && source.repo.ref ? [source.repo.ref] : [], dirty: false });
      }
    }
    ...
    const nameCollision = request.target.kind === 'repo'
      ? (await this.deps.readTargetFile(request.target.repoPathOrUrl, workflowPath(request.target.name))) !== null
      : await this.deps.localWorkflows.read(profileId, request.target.name).then(() => true).catch((error) => {
        if (error instanceof WorkflowPromotionError || (error as { code?: string }).code === 'WORKFLOW_NOT_FOUND') return false;
        throw error;
      });
  ...
  private async apply(request: ApplyRequest, profileId: string): Promise<ApplyData> {
    ...
    try { document = sourceDocument ?? await this.buildDocument(plan!, run, pins, request.hooks, profileId); }
    catch (error) {
    ...
    const warnings = plan ? await this.localFallbackWarnings(plan, pins) : [];
    const raw = `${JSON.stringify(document, null, 2)}\n`;
    const docHash = crypto.createHash('sha256').update(raw).digest('hex');
    const uniqueAdoptions = [...new Set(request.adoptRunIds ?? [])];
    if (request.target.kind === 'local' && uniqueAdoptions.length)
      throw new WorkflowPromotionError(422, 'PROMOTION_ADOPTION_UNSUPPORTED', 'Local workflows cannot adopt deployment artifacts');
    ...
    if (request.target.kind === 'repo') {
      ...
      const existing = await this.deps.localWorkflows.read(profileId, request.target.name).catch((error) => {
        if ((error as { code?: string }).code === 'WORKFLOW_NOT_FOUND') return undefined;
        throw error;
      });
      if (existing && !request.overwrite)
        throw new WorkflowPromotionError(409, 'WORKFLOW_NAME_CONFLICT', `Workflow ${request.target.name} already exists`);
      await this.deps.localWorkflows.write(profileId, request.target.name, document, existing?.docHash);
    }
  ...
  private validateTarget(target: WorkflowPromotionTarget): void {
