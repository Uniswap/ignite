...
function sourceLabel(book: AddressBookView): string {
  return book.source.kind === 'local'
    ? 'Local address book'
    : getRepoName(book.source.repoPathOrUrl);
}
...
export default function AddressBookPage() {
  ...
  const [conflict, setConflict] = useState<AddressBookMergeConflict>();
  ...
  const load = async () => {
    ...
    try {
      ...
      setEditing(
        Object.fromEntries(
          response.data.books.flatMap((book) =>
            book.entries ? [[key(book), book.entries]] : []
          )
        )
      );
      setError(undefined);
  ...
  const save = async (book: AddressBookView, force = false) => {
    ...
    try {
      if (force) {
        const body = { entries, force: true as const };
        const response =
          book.source.kind === 'local'
            ? await apiClient.request('putLocalAddressBook', { body })
            : await apiClient.request('putRepoAddressBook', {
        ...
        await saveAddressBookWithCas(
          ...
          (incoming, current) =>
            new Promise<AddressBookEntry[]>((resolve, reject) =>
              setConflict({ incoming, current, resolve, reject })
            )
        );
      }
      await load();
  ...
  const update = (
    ...
    index: number,
    patch: Partial<AddressBookEntry>
  ) => {
    const entries = [...(editing[key(book)] ?? [])];
  ...
  return (
    <div className="text-[var(--text)] grid gap-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted mt-2">
            Reusable contract addresses for deployment arguments.
          </p>
        </div>
        <button
          type="button"
      ...
      {books.map((book) => (
        <section key={key(book)} className="card-milky overflow-hidden">
          <header className="p-5 flex items-center justify-between gap-3">
            <div>
              <h2 className="font-semibold">{sourceLabel(book)}</h2>
              <p className="mono-data text-muted text-xs mt-1">
                {book.bookHash.slice(0, 12)}
                {book.writable ? '' : ' · read-only'}
              </p>
            </div>
            {book.writable && !book.error && (
              <button
                type="button"
                className="btn btn-sm btn-primary"
                onClick={() => void save(book)}
              >
                <Save size={14} /> Save
              </button>
            )}
          ...
          {book.error ? (
            <div className="px-5 pb-5 grid gap-3">
              <p className="text-sm text-err">
                {sanitizeDisplayText(book.error)}
              </p>
              {book.writable && (
                <button
                  type="button"
                  className="btn btn-secondary justify-self-start"
                  onClick={() => void save(book, true)}
                >
                  Reset book
                </button>
              )}
              ...
              <table className="w-full text-sm">
                <thead className="text-left text-muted">
                  <tr>
                    ...
                    <th className="px-3 py-2">Global address</th>
                    <th className="px-3 py-2">Per-chain overrides</th>
                    <th className="px-3 py-2">Note</th>
                    {book.writable && <th className="px-5 py-2" />}
                  </tr>
                </thead>
                <tbody>
                  {(editing[key(book)] ?? []).map((entry, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        {book.writable ? (
                          <input
                            className="input-glass mono-data w-80"
                            value={entry.address ?? ''}
                            onChange={(event) =>
                              update(book, index, {
                                address:
                          ...
                          <span className="mono-data">{entry.address}</span>
                        )}
                      </td>
                      <td className="px-3 py-2">
                        {book.writable ? (
                          <input
                            className="input-glass mono-data w-60"
                            placeholder="1=0x..., 10=0x..."
                            value={Object.entries(entry.perChain ?? {})
                      ...
                      <td className="px-3 py-2">
                        {book.writable ? (
                          ...
                          sanitizeDisplayText(entry.note ?? '')
                        )}
                      </td>
                      {book.writable && (
                        <td className="px-5 py-2">
                          <button
                            ...
                            onClick={() =>
                              setEditing({
                                ...editing,
                                [key(book)]: (editing[key(book)] ?? []).filter(
                                  (_, item) => item !== index
                                ),
                              })
                            }
