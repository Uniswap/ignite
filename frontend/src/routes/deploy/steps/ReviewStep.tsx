...
export default function ReviewStep({ plan }: ReviewStepProps) {
  ...
  return (
    <section className="grid gap-4">
      ...
      {bookResolutions && Object.keys(bookResolutions).length > 0 && <div className="card-milky p-4 grid gap-3"><div><h3 className="font-semibold">Book addresses</h3><p className="text-sm text-muted">These addresses are frozen into the launch plan.</p></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="text-left text-muted"><tr><th className="py-1 pr-3">Chain</th><th className="py-1 pr-3">Entry</th><th className="py-1 pr-3">Address</th><th className="py-1 pr-3">Source</th><th className="py-1">Book hash</th></tr></thead><tbody>{Object.entries(bookResolutions).flatMap(([chainId, items]) => items.map((item) => { const href = explorerAddressUrl(chains.find((chain) => chain.chainId === Number(chainId)), [], item.address); return <tr key={`${chainId}-${item.stepId}-${item.argPath}`}><td className="py-1 pr-3">{chainId}</td><td className="py-1 pr-3">{item.entry}</td><td className="py-1 pr-3 mono-data">{item.address}{href && <a href={href} target="_blank" rel="noreferrer" className="btn btn-sm btn-secondary-borderless ml-1" aria-label={`Open ${item.entry} on chain ${chainId} explorer`}><ExternalLink size={12} /></a>}</td><td className="py-1 pr-3">{item.source === 'repo' ? <span className="chip chip-info">from repo book{draft.workflowRef ? ` · ${getRepoName(draft.workflowRef.repoPathOrUrl)}` : ''}</span> : 'local'}</td><td className="py-1 mono-data">{item.bookHash.slice(0, 12)}</td></tr>; }))}</tbody></table></div></div>}
      {draft.workflowRef && (
