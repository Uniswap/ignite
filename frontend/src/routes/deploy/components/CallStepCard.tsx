...
export default function CallStepCard({ step, artifactData, onMove }: { step: DraftCallStep; artifactData: Record<string, ArtifactData>; onMove: (delta: number) => void }) {
  ...
  return <article className="card-milky p-4 grid gap-4"><header className="flex gap-2"><div className="flex-1"><h3 className="font-semibold">Contract call</h3><p className="text-xs text-muted">Runs in this position in every chain lane.</p></div><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step up" onClick={() => onMove(-1)}>↑</button><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step down" onClick={() => onMove(1)}>↓</button><button type="button" className="btn btn-sm btn-secondary" aria-label="Remove step" onClick={() => dispatch(removeCallStep(step.id))}>✕</button></header>
    ...
    {inputs.length > 0 && <section className="grid gap-3"><h4 className="font-medium">Arguments</h4>{inputs.map((input, index) => { const key = input.name || `arg${index}`; const maskedChainIds = draft.chains.filter((chainId) => Object.prototype.hasOwnProperty.call(step.argsPerChain?.[String(chainId)] ?? {}, key)); return <div key={key} className="grid gap-2"><AbiArgField {...addressBookProps} input={input} fieldKey={key} value={step.args?.[key]} maskedChainIds={maskedChainIds} eligibleSteps={argPointers} signerOptions={signerOptions} onChange={(value) => dispatch(setArg({ stepId: step.id, key, value }))} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => <div key={chainId} className="mt-2"><AbiArgField {...addressBookProps} chainId={chainId} input={input} fieldKey={key} value={step.argsPerChain?.[String(chainId)]?.[key]} eligibleSteps={argPointers} signerOptions={signerOptions.filter((option) => option.chainId === chainId)} onChange={(value) => dispatch(setChainArgOverride({ stepId: step.id, chainId, key, value }))} /></div>)}</details>}</div>; })}</section>}
