  ...
  return (
    <div className="grid gap-2">
      <Select
        ...
        options={[
          ...(allowGlobal
            ? [{ value: 'global', label: 'Use global target' }]
            : []),
          ...targets.map((item) => ({
            value: `step:${item.stepId}`,
            label: item.label,
          })),
...
export default function CallStepCard({
  step,
  artifactData,
  onMove,
}: {
  ...
  const targetStepId =
    step.target?.kind === 'step' ? step.target.stepId : undefined;
  const targetDeploy = targetStepId
    ? draft.steps.find(
        (item) => item.id === targetStepId && item.kind === 'deploy'
      )
    : undefined;
  ...
  const callTargetDeploy =
    targetDeploy?.kind === 'deploy' && targetDeploy.wraps
      ? draft.steps.find(
          (item) =>
            item.id === targetDeploy.wraps!.stepId && item.kind === 'deploy'
        )
      : targetDeploy;
  const targetAbi = (
  ...
  const setSignature = (signature: string) => {
    try {
      if (signature) parseAbiItem(`function ${signature}`);
      setSignatureError(undefined);
    } catch {
      setSignatureError('Enter a valid Solidity function signature.');
    }
    dispatch(
      setCallStepField({
        id: step.id,
        patch: { signature: signature || undefined },
      })
    );
  };
  const setTargetPerChain = (
    chainId: number,
  ...
  return (
    <article className="card-milky p-4 grid gap-4">
      <header className="flex gap-2">
        <div className="flex-1">
          ...
          <p className="text-xs text-muted">
            Runs in this position in every chain lane.
          </p>
        </div>
        <button
          type="button"
        ...
        <button
          ...
          onClick={() => dispatch(removeCallStep(step.id))}
        >
          ✕
        </button>
      </header>
            ...
            <div className="grid gap-3 mt-2">
              {draft.chains.map((chainId) => (
                <div key={chainId} className="card-milky p-3 grid gap-2">
                  ...
                  <TargetPicker
                    target={step.targetPerChain?.[String(chainId)]}
                    targets={targets}
                    allowGlobal
                    onChange={(target) => setTargetPerChain(chainId, target)}
                  />
                </div>
              ))}
            </div>
      ...
      {step.target?.kind === 'step' ? (
        <label className="grid gap-1">
          <span className="eyebrow">Function</span>
          {callTargetDeploy?.kind === 'deploy' &&
          !artifactData[callTargetDeploy.contractId] ? (
            <p className="flex items-center gap-2 text-sm text-muted">
              <Loader2 size={14} className="animate-spin" /> Loading functions…
            </p>
          ) : (
            <Select
              ...
              onValueChange={(signature) => {
                ...
                dispatch(
                  setCallStepField({
                    id: step.id,
                    patch: { signature, payable: fn?.payable },
                  })
                );
              }}
            />
        ...
        <>
          <label className="grid gap-1">
            <span className="eyebrow">Function signature</span>
            <input
              className="input-glass"
              value={step.signature ?? ''}
            ...
            />
            {signatureError && (
              <span className="text-xs text-err">{signatureError}</span>
            )}
          </label>
          <label className="flex gap-2 items-center">
            <input
              type="checkbox"
              checked={step.payable === true}
              onChange={(event) =>
                dispatch(
                  setCallStepField({
                    id: step.id,
                    patch: { payable: event.target.checked },
                  })
                )
              }
            />
                    ...
                    {draft.chains.map((chainId) => (
                      <div key={chainId} className="mt-2">
                        <AbiArgField
                          ...
                          input={input}
                          fieldKey={key}
                          value={step.argsPerChain?.[String(chainId)]?.[key]}
                          eligibleSteps={argPointers}
      ...
      <AdvancedStepSection>
        ...
        <div className="grid grid-cols-3 gap-2">
          {(['gasLimit', 'maxFeePerGas', 'maxPriorityFeePerGas'] as const).map(
            (key) => (
