...
export default function CallStepCard({ step, artifactData, onMove }: { step: DraftCallStep; artifactData: Record<string, ArtifactData>; onMove: (delta: number) => void }) {
  ...
  // A call that fulfils factory products is structural: products carry no
  // transaction of their own, so removing the call or editing its function
  // out from under them belongs to the Factory step, not this card.
  const fulfillsProducts = draft.steps.some((candidate) => {
    if (candidate.kind !== 'deploy') return false;
    const strategy = draft.deployExtras[candidate.id]?.strategy;
    return strategy?.kind === 'factory' && strategy.fulfilledBy === step.id;
  });
  ...
  const inputs = useMemo(
    () => callArgumentInputs(step.target?.kind, step.signature, targetAbi),
    [step.signature, step.target?.kind, targetAbi]
  );
  useEffect(() => {
    if (step.target?.kind !== 'step') return;
  ...
  }, [dispatch, inputs, step.args, step.argsPerChain, step.id, step.target?.kind]);
  ...
  return <article className="card-milky p-4 grid gap-4"><header className="flex gap-2"><div className="flex-1"><h3 className="font-semibold">{fulfillsProducts ? 'Factory call' : 'Contract call'}</h3><p className="text-xs text-muted">{fulfillsProducts ? 'Deploys the factory products tracked by this run.' : 'Runs in this position in every chain lane.'}</p></div><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step up" onClick={() => onMove(-1)}>↑</button><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step down" onClick={() => onMove(1)}>↓</button>{!fulfillsProducts && <button type="button" className="btn btn-sm btn-secondary" aria-label="Remove step" onClick={() => dispatch(removeCallStep(step.id))}>✕</button>}</header>
    {fulfillsProducts && step.target?.kind !== 'step' ? <section className="grid gap-2"><span className="eyebrow">Existing factory address</span><input className="input-glass" value={step.target?.kind === 'address' && step.target.address !== '0x' ? step.target.address : ''} placeholder="0x… (already deployed)" onChange={(event) => dispatch(setCallStepField({ id: step.id, patch: { target: { kind: 'address', address: (event.target.value || '0x') as `0x${string}` } } }))} /><span className="text-xs text-muted">The factory this run calls — the same address the Factory step shows.</span>{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain address</summary><div className="grid gap-3 mt-2">{draft.chains.map((chainId) => { const override = step.targetPerChain?.[String(chainId)]; return <label key={chainId} className="card-milky p-3 grid gap-1"><span className="font-medium">{chains.find((chain) => chain.chainId === chainId)?.name ?? chainId}</span><input className="input-glass" value={override?.kind === 'address' && override.address !== '0x' ? override.address : ''} placeholder="Use global address" onChange={(event) => setTargetPerChain(chainId, event.target.value ? { kind: 'address', address: event.target.value as `0x${string}` } : undefined)} /></label>; })}</div></details>}</section> : <section className="grid gap-2"><span className="eyebrow">Target</span><TargetPicker target={step.target} targets={targets} onChange={(target) => dispatch(setCallStepField({ id: step.id, patch: { target: target ?? null } }))} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain target</summary><div className="grid gap-3 mt-2">{draft.chains.map((chainId) => <div key={chainId} className="card-milky p-3 grid gap-2"><span className="font-medium">{chains.find((chain) => chain.chainId === chainId)?.name ?? chainId}</span><TargetPicker target={step.targetPerChain?.[String(chainId)]} targets={targets} allowGlobal onChange={(target) => setTargetPerChain(chainId, target)} /></div>)}</div></details>}</section>}
    {step.target?.kind === 'step' ? <label className="grid gap-1"><span className="eyebrow">Function</span>{callTargetDeploy?.kind === 'deploy' && !artifactData[callTargetDeploy.contractId] ? <p className="flex items-center gap-2 text-sm text-muted"><Loader2 size={14} className="animate-spin" /> Loading functions…</p> : <Select value={step.signature} requireSelection placeholder="Choose a function" options={functions.map((item) => ({ value: item.signature, label: item.signature }))} onValueChange={(signature) => { const fn = functions.find((item) => item.signature === signature); dispatch(setCallStepField({ id: step.id, patch: { signature, payable: fn?.payable } })); }} />}</label> : fulfillsProducts ? <div className="grid gap-1"><span className="eyebrow">Function</span><p className="mono-data text-sm">{step.signature}</p><span className="text-xs text-muted">Product steps read their addresses from this function&apos;s outputs; change it in the Factory step.</span></div> : <><label className="grid gap-1"><span className="eyebrow">Function signature</span><input className="input-glass" value={step.signature ?? ''} placeholder="transfer(address,uint256)" onChange={(event) => setSignature(event.target.value)} />{signatureError && <span className="text-xs text-err">{signatureError}</span>}</label><label className="flex gap-2 items-center"><input type="checkbox" checked={step.payable === true} onChange={(event) => dispatch(setCallStepField({ id: step.id, patch: { payable: event.target.checked } }))} />Payable</label></>}
    {inputs.length > 0 && <section className="grid gap-3"><h4 className="font-medium">Arguments</h4>{inputs.map((input, index) => { const key = input.name || `arg${index}`; return <div key={key} className="grid gap-2"><AbiArgField input={input} fieldKey={key} value={step.args?.[key]} eligibleSteps={argPointers} signerOptions={signerOptions} onChange={(value) => dispatch(setArg({ stepId: step.id, key, value }))} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => <div key={chainId} className="mt-2"><AbiArgField input={input} fieldKey={key} value={step.argsPerChain?.[String(chainId)]?.[key]} eligibleSteps={argPointers} signerOptions={signerOptions.filter((option) => option.chainId === chainId)} onChange={(value) => dispatch(setChainArgOverride({ stepId: step.id, chainId, key, value }))} /></div>)}</details>}</div>; })}</section>}
