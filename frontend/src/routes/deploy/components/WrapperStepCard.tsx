...
export function initializerArgumentMaskedChainIds(chains: number[], argsPerChain: DraftDeployStep['argsPerChain'], initializerArg: string, argument: string): number[] {
  return chains.filter((chainId) => {
    const override = argsPerChain?.[String(chainId)]?.[initializerArg];
    if (override === undefined) return false;
    if (!isEncode(override)) return true;
    return Object.prototype.hasOwnProperty.call(override.$encode.args ?? {}, argument);
  });
}
...
export default function WrapperStepCard({ step, implementationAbi, onMove }: { step: DraftDeployStep; implementationAbi?: unknown[]; onMove: (delta: number) => void }) {
  ...
  const addressBookProps = { selectedChainIds: draft.chains, contextualBookSourceKey: draft.workflowRef ? `repo:${draft.workflowRef.repoPathOrUrl}` : 'local', allowBookLink: true, chains };
  ...
  return <article className="card-milky p-4 grid gap-4">
    ...
    {type?.params.map((param) => { const argName = type.synthesis?.constructorArgs.find((arg) => arg.from === 'param' && arg.param === param.key)?.name ?? param.key; const input: AbiInput = { name: param.label, type: param.type === 'number' ? 'uint256' : param.type === 'boolean' ? 'bool' : param.type === 'select' ? 'string' : param.type }; const setParam = (value: unknown, chainId?: number) => chainId === undefined ? dispatch(setArg({ stepId: step.id, key: argName, value })) : dispatch(setChainArgOverride({ stepId: step.id, chainId, key: argName, value })); const field = (value: unknown, chainId?: number, maskedChainIds?: number[]) => param.type === 'select' ? <label className="grid gap-1"><span className="text-sm font-medium">{param.label}</span><Select value={typeof value === 'string' ? value : undefined} options={param.options ?? []} onValueChange={(next) => setParam(next, chainId)} /></label> : <AbiArgField {...addressBookProps} chainId={chainId} maskedChainIds={maskedChainIds} input={input} fieldKey={argName} value={value} eligibleSteps={param.type === 'address' ? pointerSteps : undefined} signerOptions={chainId === undefined ? signerOptions : signerOptions.filter((option) => option.chainId === chainId)} autoDefault={param.type === 'boolean' && chainId === undefined} onChange={(value) => setParam(value, chainId)} />; const maskedChainIds = draft.chains.filter((chainId) => Object.prototype.hasOwnProperty.call(step.argsPerChain?.[String(chainId)] ?? {}, argName)); return <div key={param.key} className="grid gap-2">{field(step.args?.[argName], undefined, maskedChainIds)}{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => <div key={chainId} className="mt-2">{field(step.argsPerChain?.[String(chainId)]?.[argName], chainId)}</div>)}</details>}</div>; })}
    {initializerArg && <section className="grid gap-3"><label className="grid gap-1"><span className="eyebrow">Initializer</span><Select value={selected} options={[{ value: '', label: 'No initialization (empty calldata)' }, ...functions.map((item) => ({ value: item.signature, label: item.signature }))]} onValueChange={setInitializer} /></label>{selectedFunction?.item.inputs?.map((input, index) => { const key = input.name || `arg${index}`; const global = isEncode(step.args?.[initializerArg]) ? step.args![initializerArg] as Encode : undefined; const maskedChainIds = initializerArgumentMaskedChainIds(draft.chains, step.argsPerChain, initializerArg, key); return <div key={key} className="grid gap-2"><AbiArgField {...addressBookProps} maskedChainIds={maskedChainIds} input={input} fieldKey={key} value={global?.$encode.args?.[key]} eligibleSteps={pointerSteps} signerOptions={signerOptions} onChange={(value) => setInitializerArg(key, value)} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => { const override = step.argsPerChain?.[String(chainId)]?.[initializerArg]; return <div key={chainId} className="mt-2 grid gap-1"><AbiArgField {...addressBookProps} chainId={chainId} input={input} fieldKey={key} value={isEncode(override) ? override.$encode.args?.[key] : undefined} eligibleSteps={pointerSteps} signerOptions={signerOptions.filter((option) => option.chainId === chainId)} onChange={(value) => setInitializerArg(key, value, chainId)} />{override !== undefined && <button type="button" className="btn btn-sm btn-secondary justify-self-start" onClick={() => dispatch(setChainArgOverride({ stepId: step.id, chainId, key: initializerArg, value: undefined }))}>Use global</button>}</div>; })}</details>}</div>; })}</section>}
