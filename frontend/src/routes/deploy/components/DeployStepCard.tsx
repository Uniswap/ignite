...
export default function DeployStepCard({
  step,
  data,
  onMove,
}: {
  ...
  const [contractTypePending, setContractTypePending] = useState(false);
  const contractTypeRequest = useRef(0);
  useEffect(() => {
    let cancelled = false;
    void apiClient
  ...
  const sourcePath =
    contract?.origin === 'contract-type' ? undefined : contract?.sourcePath;
  const inputs =
    (
  ...
  const selectedWrapper = draft.steps.find(
    (candidate): candidate is DraftDeployStep =>
      candidate.kind === 'deploy' && candidate.wraps?.stepId === step.id
  );
  ...
  return (
    <article className="card-milky p-4 grid gap-4">
        ...
        <div className="flex-1">
          <h3 className="font-semibold">
            {contract?.contractName ??
              decodeUrlEncodingForDisplay(step.contractId)}
          </h3>
          <p className="mono-data text-muted">
            {sourcePath ? decodeUrlEncodingForDisplay(sourcePath) : undefined}
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="grid gap-1">
        <label className="grid gap-1">
          ...
          <Select
            value={
              selectedWrapper
                ? `plugin:${selectedWrapper.wraps?.contractTypePluginId}`
                : 'immutable'
            }
            options={[
              ...
              ...contractTypes.map((item) => ({
                ...
                label: `${item.label} (${item.versionLabel})`,
              })),
            ]}
            onValueChange={(value) => {
              const request = ++contractTypeRequest.current;
              setContractTypeError(undefined);
              if (value === 'immutable') {
                setContractTypePending(false);
                dispatch(setContractTypeSelectionPending(false));
                dispatch(selectContractType({ implementationStepId: step.id }));
                return;
              }
              const type = contractTypes.find(
                (item) => item.pluginId === value.slice('plugin:'.length)
              );
              const synthesis = type?.synthesis;
              if (!type || !synthesis) return;
              setContractTypePending(true);
              dispatch(setContractTypeSelectionPending(true));
              void apiClient
                .request('getContractTypeArtifact', {
                  params: {
                ...
                .then((response) => {
                  if (request !== contractTypeRequest.current) return;
                ...
                .catch((reason) => {
                  if (request === contractTypeRequest.current)
                    setContractTypeError(
                      reason instanceof ApiError
                        ? (reason.body.message ?? reason.message)
                        : String(reason)
        ...
        {requiresGrant.map((pluginId) => (
          ...
          >
            {pluginId} — grant contract bytecode access to use this contract
            type.
          </button>
        ))}
        {contractTypePending && (
          <p className="text-xs text-muted">Loading contract type…</p>
        )}
        {contractTypeError && (
          <p className="text-xs text-err">{contractTypeError}</p>
        )}
      </section>
      <StrategySection stepId={step.id} />
      <LibrariesSection stepId={step.id} libraries={libraryReferences(data)} />
      <section className="grid gap-3">
        <h4 className="font-medium">Constructor arguments</h4>
        {!data && (
          <p className="flex items-center gap-2 text-sm text-muted">
            <Loader2 size={14} className="animate-spin" /> Loading artifact…
          </p>
        )}
      ...
      <AdvancedStepSection>
        {(
          data?.abi as
            Array<{ type?: string; stateMutability?: string }> | undefined
        )?.find((entry) => entry.type === 'constructor')?.stateMutability ===
          'payable' && (
          <label className="grid gap-1">
