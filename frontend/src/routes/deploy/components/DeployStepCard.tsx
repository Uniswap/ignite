...
export default function DeployStepCard({ step, data, onMove }: { step: DraftDeployStep; data?: ArtifactData; onMove: (delta: number) => void }) {
  ...
  return <article className="card-milky p-4 grid gap-4">
    ...
    <section className="grid gap-3"><h4 className="font-medium">Constructor arguments</h4>{!data && <p className="flex items-center gap-2 text-sm text-muted"><Loader2 size={14} className="animate-spin" /> Loading artifact…</p>}{inputs.map((input, index) => { const key = input.name || `arg${index}`; return <div key={key} className="grid gap-2"><AbiArgField input={input} fieldKey={key} value={step.args?.[key]} autoDefault eligibleSteps={eligible} signerOptions={signerOptions} explorerOptions={explorerOptions} onChange={(value) => dispatch(setArg({ stepId: step.id, key, value }))} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary><div className="grid gap-3 mt-2">{draft.chains.map((chainId) => <ChainOverrideCard key={chainId} chainId={chainId}><AbiArgField input={input} fieldKey={key} value={step.argsPerChain?.[String(chainId)]?.[key]} eligibleSteps={eligible} signerOptions={signerOptions.filter((option) => option.chainId === chainId)} explorerOptions={explorerOptions.filter((option) => option.chainId === chainId)} onChange={(value) => dispatch(setChainArgOverride({ stepId: step.id, chainId, key, value }))} /></ChainOverrideCard>)}</div></details>}</div>; })}</section>
