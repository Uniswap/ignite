...
export default function DeployStepCard({ step, data, onMove }: { step: DraftDeployStep; data?: ArtifactData; onMove: (delta: number) => void }) {
  ...
  return <article className="card-milky p-4 grid gap-4">
    ...
    <section className="grid gap-3"><h4 className="font-medium">Constructor arguments</h4>{!data && <p className="flex items-center gap-2 text-sm text-muted"><Loader2 size={14} className="animate-spin" /> Loading artifact…</p>}{inputs.map((input, index) => { const key = input.name || `arg${index}`; const maskedChainIds = draft.chains.filter((chainId) => Object.prototype.hasOwnProperty.call(step.argsPerChain?.[String(chainId)] ?? {}, key)); return <div key={key} className="grid gap-2"><AbiArgField {...addressBookProps} input={input} fieldKey={key} value={step.args?.[key]} maskedChainIds={maskedChainIds} autoDefault eligibleSteps={eligible} signerOptions={signerOptions} onChange={(value) => dispatch(setArg({ stepId: step.id, key, value }))} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => <div key={chainId} className="mt-2"><AbiArgField {...addressBookProps} chainId={chainId} input={input} fieldKey={key} value={step.argsPerChain?.[String(chainId)]?.[key]} eligibleSteps={eligible} signerOptions={signerOptions.filter((option) => option.chainId === chainId)} onChange={(value) => dispatch(setChainArgOverride({ stepId: step.id, chainId, key, value }))} /></div>)}</details>}</div>; })}</section>
